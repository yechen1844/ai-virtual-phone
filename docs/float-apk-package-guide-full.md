...
## 11. 断点 E：前后端主动消息「撞车」（双生成浪费资源）——✅ 已修复并部署

### 现象

定时/主动类消息，每个到期周期前端(本地)与后端(push-generate/云端)**同刻各生成一次**，用户只见前端那条，云端那条被消费去重吞掉、白耗一次 LLM 生成。

### 竞态根因

前端触发批 N 后调 `cancelBailoutPrefix(idle:<ruleId>:)` 撤销云端同计划 `status=pending` 的 job；一旦云端抢先 claim（`pending→running`），前端撤销匹配不到 pending → 扑空 → 双生成。**没有先后保证时，任何"先写标记后查"都不可靠。**

### 修复（已实施并部署）

> ⚠️ 定时主动消息是**长期规则**（"char 间隔 N 未回复就主动说一句"），**计划本体绝不删除**。绝不能把本轮到期当一次性任务去 cancel/删除。

`push-generate` 对定时/主动类 job（`kind=timed_task` 或 `trigger_key` 以 `idle:`/`timedwake:`/`periodcare:` 开头）在 **claim（`where status=pending`）之前** **`sleep(120_000)`**，再走原原子抢锁：

- 前端活着 → 前端 2 分钟内先手生成并撤销本轮 pending job → 120s 后抢锁落空 → 只出前端 1 条；

- 前端被杀 → 无人撤销 → 120s 后仍 pending → 后端兜底 1 条；

- 计划保留：各周期用批次号 N，下周期 N+1 照常，**计划不删**。

### 关键文件（真正上线源）

`supabase/functions/push-generate/index.ts`（不是 `public/ai-phone-push/` 那份）。

### 部署方式（Management API，单函数）
...
multipart/form-data:
  ...
  file      = 整个 supabase/functions/push-generate/index.ts (application/typescript)
```

- **坑**：`metadata` 别用 PowerShell `-F "metadata=..."` 直传（会被拆成数组、json 解析失败）；用 Node 的 `FormData`+`Blob` 最稳。

- 成功标志：HTTP `201`，返回 `{"slug":"push-generate","version":N,"status":"ACTIVE"}`（version 递增）。

- **已验证**：已部署到个人云 → `push-generate` version 4 / ACTIVE，改动已生效。

### 验证

前端开着 float 建一个短周期定时主动消息 → 到点**只出前端 1 条**，云端该批未重复 `done`（或已 `cancelled`）；杀掉 float → 120s 后云端兜底 1 条并进聊天；计划长期仍在、每周期恰 1 条。

### 口径（写死）

- 主动/定时类判定：`timed_task` 或 `trigger_key` 前缀 `idle:`/`timedwake:`/`periodcare:`。

- 后端对这些 claim 前 sleep 120s；靠 `where status=pending` 抢锁达成"前端活=前端出，前端死=后端兜底"。

- **计划不删**，只对"本轮"让路。

- **真正部署源** **`supabase/functions/`，改完必须 Management API** **`functions/deploy`** **重新部署才算生效。**
