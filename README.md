...
> 本仓库基于上游 `main` / `test` 分支二次开发，**仅用于作者个人自部署与自用**。其中包含大量针对个人场景的私有二改（安卓壳、离线推送、星露谷联动、阅读摘要、云同步等），不代表上游官方版本，**bug 甚多，仅供大家参考**。若你 fork 或分发本仓库，请遵守上游 AGPL-3.0 的义务：保留原作者版权与许可证、注明你做的修改、以相同许可公开，并尽量指向上游仓库。如需官方功能与最新稳定版，请以上游仓库为准。
...
- 屏幕速聊（iOS）：通过辅助触控截取当前屏幕，在系统弹窗里与角色连续对话，并同步回唯一聊天窗口
...
| `SUPABASE_URL` / `SUPABASE_SERVICE_ROLE_KEY`                                                          | 站点账号、社区功能和现实桥邮件自动化使用的 Supabase 项目（服务端专用，勿放进 NEXT\_PUBLIC）；应用内创建的个人云不需要把密钥填到这里 |
...
角色的回复直接显示在系统弹窗里；你可以在弹窗中继续回复。每一轮对话都会按顺序写回
该角色原有的唯一聊天窗口，不会每 15/30 轮另开一段会话。

使用前请先在 **设置 → 云服务部署**完成或重新执行一次个人云部署。屏幕速聊要求
个人云 Schema v3；旧版个人云不会被静默复用，页面会提示重新部署。部署完成后：

1. 打开 **现实桥 → 屏幕速聊**，开启功能并选择一个角色；
2. 在第三步点“一键导入屏幕速聊”，导入项目提供的通用 iCloud 快捷指令；
3. 按导入问题填写页面显示的“接口地址”和“令牌”。共享快捷指令本身不包含你的地址、
   令牌或角色资料；
4. 到 iPhone **设置 → 辅助功能 → 触控 → 辅助触控 → 轻点两下**，选择“屏幕速聊”；
5. 首次运行时允许“截屏”“访问网络”和“显示提醒”等系统权限。

如果一键导入不可用，页面仍保留完整的手动搭建步骤。该功能不需要 Resend，也不需要
站点运营库；它只访问用户自己创建的个人云项目。截图原图不会写入 Supabase 数据库或
Storage，只存在于本次 Edge Function 内存和本次模型请求中。个人云仅暂存尚未合并回
小手机的文本轮次；应用启动或从后台回到前台时会立即补拉，而不是持续高频轮询。
...
本项目为 **xiaolongbao0709** 的原始作品（上游仓库：[github.com/xiaolongbao0709/ai-virtual-phone](https://github.com/xiaolongbao0709/ai-virtual-phone)）的个人自部署 + 二次开发 fork，上游采用 **GNU AGPL-3.0-only** 许可。本 fork 为作者自用，**bug 甚多，仅供大家参考**，不视为稳定可用版本。若你对本 fork 再分发或再修改，请遵守 AGPL-3.0：保留原作者版权与许可证、注明你所作的修改、以相同许可证公开源码，并尽量指向上游官方仓库。上游项目为独立实现，部分产品设计和系统抽象受 SillyTavern 启发，包括预设、正则处理、世界书 / lorebook / WorldInfo 等概念。
