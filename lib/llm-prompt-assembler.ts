...
export function assemblePromptPayload(input: AssemblerInput): LLMMessage[] {
    ...
    if (preset) {
        ...
        engine.movieSummary = input.movieSummary ?? "";
        engine.sceneSubtitleWindow = input.sceneSubtitleWindow ?? "";
        engine.frameHint = input.frameHint ?? "";
        engine.sceneBoundaries = input.sceneBoundaries ?? "";
        engine.moviePosition = input.moviePosition ?? "";
