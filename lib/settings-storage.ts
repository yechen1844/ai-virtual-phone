...
/** 找出目标预设缺失的内置条目（按 identifier 对比）。 */
export function findMissingBuiltinPrompts(presetId: string): Array<{
    identifier: string;
    name: string;
    tags: string[];
    enabled: boolean;
    ...
    if (!target) return [];
    const builtin = presets.find(p => p.builtIn) ?? createBuiltinPreset();
    const existingIds = new Set((target.prompts ?? []).map(p => p.identifier));
    const missing = (builtin.prompts ?? []).filter(p => !existingIds.has(p.identifier));
...
/**
 * 将指定的内置条目同步到目标预设。
 * - 按 identifier 找到内置条目的完整定义
 * - 插入位置：参照内置 prompt_order，找到目标预设里已存在的、往前最近的条目，插到它后面
 * - 同步更新 prompt_order
 * 返回实际添加的条目数。
 */
export function syncPresetFromBuiltin(presetId: string, identifiers: string[]): number {
    const presets = loadPresets();
    const targetIdx = presets.findIndex(p => p.id === presetId);
    if (targetIdx < 0) return 0;
    const target = presets[targetIdx];
    const builtin = presets.find(p => p.builtIn) ?? createBuiltinPreset();
    const builtinPrompts = builtin.prompts ?? [];
    const builtinOrder = builtin.prompt_order ?? [];

    const existingIds = new Set((target.prompts ?? []).map(p => p.identifier));
    let added = 0;

    const newPrompts = [...(target.prompts ?? [])];
    const newOrder = [...(target.prompt_order ?? [])];

    for (const id of identifiers) {
        if (existingIds.has(id)) continue;
        const builtinPrompt = builtinPrompts.find(p => p.identifier === id);
        if (!builtinPrompt) continue;

        // 找到在内置 order 中该条目前面最近的、目标预设已存在的条目
        const builtinIdx = builtinOrder.findIndex(e => e.identifier === id);
        let insertAfterIdx = -1;
        for (let i = builtinIdx - 1; i >= 0; i--) {
            const prevId = builtinOrder[i].identifier;
            const orderIdx = newOrder.findIndex(e => e.identifier === prevId);
            if (orderIdx >= 0) {
                insertAfterIdx = orderIdx;
                break;
            }
        }

        // 插入 prompt 条目
        newPrompts.push(builtinPrompt);
        existingIds.add(id);

        // 插入 order 条目
        const orderEntry = builtinOrder.find(e => e.identifier === id);
        if (orderEntry) {
            if (insertAfterIdx >= 0) {
                newOrder.splice(insertAfterIdx + 1, 0, orderEntry);
            } else {
                newOrder.push(orderEntry);
            }
        }
    ...
    if (added > 0) {
        presets[targetIdx] = { ...target, prompts: newPrompts, prompt_order: newOrder };
        savePresets(presets);
    }
...
export function removeApiConfigReferences(apiConfigId: string): void {
    ...
    const auxFields = [
        ...
        "complexMemoryApiConfigId",
        "complexRerankApiConfigId",
    ] as const;
...
export function resolveAuxiliaryApiConfig(
    field: "memorySummaryApiConfigId" | "embeddingApiConfigId" | "mascotApiConfigId" | "reasoningTranslateApiConfigId" | "qaApiConfigId" | "complexMemoryApiConfigId" | "complexRerankApiConfigId"
): ApiConfig | null {
    const config = loadBindingConfig();
    const apiConfigs = loadApiConfigs();
