...
export type ComplexMemoryConfig = {
  ...
  autoGenerationEnabled: boolean;    // 已弃用（保留字段兼容旧配置）：后台自动生成由 autoSummarizeEnabled 统一控制。
  autoSummarizeEnabled: boolean;     // 自动总结总开关：后台自动生成 事件/日记/周期/核心 记忆；关闭后仅一键迁移/手动可生成。
  // 电压
